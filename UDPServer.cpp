...
UDPServer::UDPServer(const Config& config) : config_(config) {
    udp_data_q_ = std::make_shared<SPSCQueue<UDPData>>(10);
    udp_imu_q_ = std::make_shared<SPSCQueue<ImuData>>(10);
}
...
bool UDPServer::revice_imu(ImuData& udp_imu) {
    return udp_imu_q_->pop(udp_imu);
}

bool UDPServer::send_cmd(UDPCmd& udp_cmd) {
